...
from broken.core import actions, apply, flatten, pydantic2typer
from broken.core.logging import log
...
class BrokenTerminal:
    ...
    class BaseModel(ABC, BaseModel):
        """A meta class for BaseModels that contains other BaseModels and will be added to aBrokenTerminal"""

        @abstractmethod
        def commands(self, typer: BrokenTerminal) -> None:
            pass
    ...
    def command(self,
        ...
        **kwargs,
        ...
        if issubclass(_class, BaseModel):
            ...
            if issubclass(_class, BrokenTerminal.BaseModel):
    ...
    def simple(*commands: Iterable[Callable]) -> None:
        app = BrokenTerminal().release_repl()
        apply(app.command, commands)
        return app(sys.argv[1:])

    @staticmethod
    def release(main: Callable, *others: Iterable[Callable]) -> None:
        app = BrokenTerminal(description=(
            "📦 [bold orange3]BrokenTerminal's[/] Multiple entry points handler for releases executables\n\n"
    ...
    def repl_prompt(self) -> bool:
        try:
            ...
            log.trace("BrokenTerminal Repl exit KeyboardInterrupt")
        return False
    ...
    def __call__(self, *args: Iterable[Any]) -> None:
        ...
        for index in itertools.count():
            ...
            try:
                ...
                log.trace("Skipping SystemExit on BrokenTerminal")
            except KeyboardInterrupt:
                log.success("BrokenTerminal exit KeyboardInterrupt")
            except Exception as error:
                if (not self.repl):
                    raise error
                console.print_exception(); print() # noqa
                log.error(f"BrokenTerminal exited with error: {repr(error)}")
